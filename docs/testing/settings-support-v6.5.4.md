...
状态：v6.5.4 发布前本地验收记录。初始基于工作区 v6.5.2 开发，发布前已合入远端 v6.5.3 的 Explicit 标记修复。
...
- 完整收款地址首六位与末六位使用不同颜色，中间部分保持常规文字。深浅主题使用明确的不透明底色以保证文字对比度；强制颜色模式使用系统颜色。分段不引入空格、截断或复制内容变化。
...
已有的 `.github/RELEASE_TEMPLATE.md` 和版本索引写作规范改动单独保留，未纳入功能提交。远端发布状态以 GitHub Release 为准。
