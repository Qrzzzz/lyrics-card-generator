...
<p>
  ...
  <a href="./docs/releases/v3.6.1.ja.md">リリースノート</a> ·
  <a href="#主な機能">主な機能</a> ·
...
* 推奨インストーラー：`Lyrics Card Generator Setup 3.6.1.exe`
* ポータブル版：`Lyrics Card Generator-3.6.1-portable.exe`
...
* デスクトップのカスタムタイトルバーで最大化 / 復元状態が同期しない問題を修正しました。
* Acrylic デスクトップタイトルバーに自然に消えるガラスぼかし、適切な重なり順、安定した閉じるボタン hover を追加しました。
* タイトルバーのウィンドウ操作ボタンに多言語かつアクセシブルな文言を追加しました。
* README のダウンロード入口とリリースノートリンクを修正し、歌詞カードの PNG 出力は変更していません。
...
- 横向きレイアウトはセーフエリアを基準に再構築され、ジャケット列、コンテンツ列、下部情報がより安定
- 縦向きのカスタムサイズでは、実際の DOM 測定に基づく自動高さ調整に対応
- 高解像度 PNG 画像として書き出し可能

### 📝 歌詞レイアウトと翻訳

- 原文歌詞と翻訳を横並びで表示
- 現在のインターフェース言語に基づいて、原文 / 翻訳を自動分割
- 簡体字中国語、繁体字中国語、英語、フランス語、日本語、スペイン語への翻訳に対応
- OpenAI Chat Completions 互換の AI 歌詞翻訳に対応
- プロバイダーの Base URL、モデル、API Key、翻訳スタイル、Reasoning、ストリーミング出力を設定可能

### 🎵 楽曲情報の取り込み

- Spotify、Apple Music、NetEase Cloud Music、QQ Music のリンク解析に対応
- ローカル MP3 / FLAC のメタデータ解析に対応
- タイトル、アーティスト、アルバム、カバー、埋め込み歌詞の読み取りを試行
- 曲名、アーティスト、カバー、歌詞を手動入力可能
- ローカルカバー画像のアップロードに対応

### 🌈 ビジュアルとカスタマイズ

- カバー画像から色を抽出し、グラデーション背景を生成
- プラットフォームロゴ、共有者名、生成ウォーターマークに対応
- 源ノ角ゴシック / 源ノ明朝の 2 種類のフォント方案を搭載
- カスタム CJK フォント、西文フォント、システムフォント選択に対応
- 実際の歌詞フォントでプレビュー可能

### 🌍 多言語とアップデート

- 簡体字中国語 / 繁体字中国語 / English / Français / 日本語 / Español のインターフェース切り替えに対応
- GitHub Releases から新バージョンを確認可能
