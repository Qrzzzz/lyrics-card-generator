...
<p>
  ...
  <a href="./docs/releases/v3.7.0.ja.md">リリースノート</a> ·
  <a href="#主な機能">主な機能</a> ·
...
* 推奨インストーラー：`Lyrics Card Generator Setup 3.7.0.exe`
* ポータブル版：`Lyrics Card Generator-3.7.0-portable.exe`
...
* デスクトップ背景画像のパス検証を強化し、アプリが生成した安全なファイル名のみを受け付けます。
* 非標準レスポンス、プレーンテキストエラー、不正な JSON、空レスポンスに対する AI 翻訳のフォールバックを改善しました。
* 更新入口の挙動を統一し、常に対応する GitHub Release ページを開くようにしました。
* 使われていない旧フォントパネルコードと、削除済みインストゥルメンタル入力欄の残留文言を整理しました。
* バージョン固定のリリース確認を、バージョン非依存の整合性テストに置き換えました。
...
* 原文歌詞と翻訳の並列レイアウト
* 現在のインターフェース言語に応じて原文 / 訳文を自動分割
* 簡体字中国語、繁体字中国語、英語、フランス語、日本語、スペイン語の目標訳文に対応
* OpenAI 互換 Chat Completions API を使う AI 歌詞翻訳
* プロバイダーの Base URL、モデル、API キー、翻訳スタイル、Reasoning、ストリーミング出力を設定可能

### 🎵 曲情報の取り込み

* Spotify、Apple Music、NetEase Cloud Music、QQ Music のリンク解析
* ローカル MP3 / FLAC のメタデータ解析
...
* 源ノ角ゴシック / 源ノ明朝のフォント構成
* CJK フォント、欧文フォント、システムフォントを個別に選択
* 実際の歌詞によるフォントプレビュー

### 🌍 多言語と更新

* 簡体字中国語 / 繁体字中国語 / English / Français / 日本語 / Español インターフェース
* GitHub Releases から更新を確認し、対応する Release ページを開く機能
...
ローカル Next API ルート経由でこのプロジェクトの GitHub Releases を取得し、現在のバージョンと最新リリースを比較して、対応する Release ページを開いてダウンロードアセットを選べるようにします。
