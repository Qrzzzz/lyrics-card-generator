...
一般公開されているビルドは [GitHub Releases](https://github.com/Qrzzzz/lyrics-card-generator/releases/latest) で確認できます。ローカルの v5.9.4 リリース候補は次のファイル名でパッケージ化されます。
...
* 遅延画面の失敗を局所化し、chunk を実際に再取得する再試行を追加しました。また、レジストリだけが変更された場合のシステムフォントキャッシュの最大陳腐化時間を 5 分に制限しました。ここに記載するのはローカルのリリース候補であり、一般公開状況は実際の GitHub Releases ページに従います。
