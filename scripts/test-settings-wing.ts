...
assert.match(settingsSurface, /<SettingsPageHeading/);
assert.match(settingsSurface, /testId=\{`settings-page-heading-\$\{tab\.id\}`\}/);
assert.match(settingsSurface, /title=\{tab\.label\}/);
assert.match(settingsSurface, /description=\{tab\.description\}/);
...
assert.match(lyricEditor, /<AppToast notice=\{toast\} \/>/);
...
assert.match(generalSettings, /await showSystemAlert\(/);
assert.match(generalSettings, /message: dialogCopy\.historyCheckFailedTitle/);
...
assert.match(aiSettingsSection, /<SettingsPageHeading/);
assert.doesNotMatch(aiSettingsSection, /function PageHeading/);
