...
const updateInstall = buildJob.indexOf("npm run desktop:updater-install-test");
const normalizedAssets = buildJob.indexOf("Normalize release asset filenames");
assert.ok(finalArtifactSmoke < updateInstall && updateInstall < normalizedAssets, "the actual updater must install and reopen the final Setup before publication");
...
assert.match(createSection, /\$setupAsset release\/SHA256SUMS \$sbomAsset release\/latest\.yml/, "draft creation uploads Setup, SHA256SUMS, SPDX SBOM, and updater metadata");
assert.match(buildJob, /node scripts\/verify-update-metadata\.mjs/, "update metadata is verified against tested installer bytes");
assert.equal((buildJob.match(/release\/latest\.yml/g) || []).length, 2, "update metadata is attested and transferred");
