...
const AI_UI_COPY: Record<Locale, AIUiCopy> = {
  zh: {
    ...
    aiSection: "AI 歌词翻译",
    baseUrl: "Base URL",
    baseUrlTip: "填写厂商提供的 OpenAI-compatible API 根地址。远程地址必须使用 HTTPS；HTTP 仅支持 localhost 或回环 IP。应用会自动在末尾追加 /chat/completions；如果地址已包含该路径则不会重复追加。",
    apiKey: "API Key",
  ...
  "zh-TW": {
    ...
    aiSection: "AI 歌詞翻譯",
    baseUrl: "Base URL",
    baseUrlTip: "請填入服務商提供的 OpenAI-compatible API 根網址。遠端網址必須使用 HTTPS；HTTP 僅支援 localhost 或回送 IP 位址。應用程式會自動在末尾加入 /chat/completions；若網址已包含此路徑則不會重複加入。",
    apiKey: "API Key",
  ...
  en: {
    ...
    baseUrl: "Base URL",
    baseUrlTip: "Enter the OpenAI-compatible API root supplied by your provider. Remote URLs must use HTTPS; HTTP is supported only for localhost or loopback IP addresses. /chat/completions is appended automatically unless already present.",
    apiKey: "API Key",
  ...
  fr: {
    ...
    baseUrl: "URL de base",
    baseUrlTip: "Saisissez l’URL racine de l’API compatible OpenAI fournie par votre prestataire. Les URL distantes doivent utiliser HTTPS ; HTTP est réservé à localhost et aux adresses IP de bouclage. /chat/completions est ajouté automatiquement s’il n’est pas déjà présent.",
    apiKey: "Clé API",
  ...
  ja: {
    ...
    aiSection: "AI 歌詞翻訳",
    baseUrl: "ベース URL",
    baseUrlTip: "プロバイダーが提供する OpenAI 互換 API のルート URL を入力してください。リモート URL では HTTPS が必須です。HTTP は localhost またはループバック IP でのみ使用できます。末尾に /chat/completions がない場合は自動的に追加されます。",
    apiKey: "API キー",
  ...
  es: {
    ...
    baseUrl: "URL base",
    baseUrlTip: "Introduce la URL raíz de la API compatible con OpenAI que proporciona tu proveedor. Las direcciones remotas deben usar HTTPS; HTTP solo se admite para localhost o direcciones IP de bucle invertido. /chat/completions se añade automáticamente si aún no está incluido.",
    apiKey: "Clave API",
