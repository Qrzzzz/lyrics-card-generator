...
const COPY: Record<Locale, AIPromptUiCopy> = {
  zh: {
    ...
    reset: "重置", resetAll: "重置全部", resetAllConfirm: "将所有默认预设、严格格式规则和自定义预设恢复到各自的初始内容？", resetRulesConfirm: "将严格格式规则恢复为 4.6.0 默认内容？",
    ...
    resetPresetConfirm: "将这个预设恢复为最初的标题和提示词？",
  ...
  "zh-TW": {
    ...
    unlockConfirmSecond: "再次確認：修改後 AI 可能不再只輸出譯文原文。是否繼續？", reset: "重設", resetAll: "全部重設", resetAllConfirm: "將所有預設、嚴格格式規則與自訂預設還原為各自的初始內容？", resetRulesConfirm: "將嚴格格式規則恢復為 4.6.0 預設內容？",
    ...
    deletePresetConfirm: "刪除這個預設？此操作會立即儲存。", resetPresetConfirm: "將這個預設還原為最初的標題與提示詞？",
  ...
  en: {
    ...
    unlockConfirmSecond: "Confirm again: AI may stop returning only the translated lyrics after this change. Continue?", reset: "Reset", resetAll: "Reset all", resetAllConfirm: "Restore every default preset, the strict format rules, and each custom preset to its initial content?", resetRulesConfirm: "Restore the strict format rules to the 4.6.0 defaults?",
    ...
    deletePresetConfirm: "Delete this preset? The change is saved immediately.", resetPresetConfirm: "Restore this preset's initial title and prompt?",
  ...
  fr: {
    ...
    unlockConfirmSecond: "Confirmez à nouveau : l’IA pourrait ne plus renvoyer uniquement la traduction. Continuer ?", reset: "Réinitialiser", resetAll: "Tout réinitialiser", resetAllConfirm: "Rétablir chaque préréglage, les règles de format et chaque préréglage personnalisé à leur contenu initial ?", resetRulesConfirm: "Rétablir les règles de format par défaut de la 4.6.0 ?",
    ...
    deletePresetConfirm: "Supprimer ce préréglage ? La modification sera enregistrée immédiatement.", resetPresetConfirm: "Rétablir le titre et le prompt initiaux de ce préréglage ?",
  ...
  ja: {
    ...
    unlockConfirmSecond: "再確認：変更後は翻訳本文以外が出力される可能性があります。続けますか？", reset: "リセット", resetAll: "すべてリセット", resetAllConfirm: "すべての既定プリセット、形式ルール、カスタムプリセットをそれぞれの初期内容に戻しますか？", resetRulesConfirm: "形式ルールを 4.6.0 の既定内容に戻しますか？",
    ...
    deletePresetConfirm: "このプリセットを削除しますか？変更はすぐ保存されます。", resetPresetConfirm: "このプリセットのタイトルとプロンプトを初期内容に戻しますか？",
  ...
  es: {
    ...
    unlockConfirmSecond: "Confirma de nuevo: la IA podría dejar de devolver únicamente la traducción. ¿Continuar?", reset: "Restablecer", resetAll: "Restablecer todo", resetAllConfirm: "¿Restaurar todos los preajustes predeterminados, las reglas de formato y cada preajuste personalizado a su contenido inicial?", resetRulesConfirm: "¿Restaurar las reglas de formato predeterminadas de 4.6.0?",
    ...
    deletePresetConfirm: "¿Eliminar este preajuste? El cambio se guardará de inmediato.", resetPresetConfirm: "¿Restaurar el título y el prompt iniciales de este preajuste?",
