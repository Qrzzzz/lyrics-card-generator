...
<p>
  ...
  <a href="./docs/releases/v5.6.0.en.md">Release Notes</a> ·
...
* Recommended installer: `Lyrics Card Generator Setup 5.6.0.exe`
* Portable version: `Lyrics Card Generator-5.6.0-portable.exe`
...
* Lyrics Examples now uses a Settings-style top header with a close button in the upper-right. The oversized bottom app-title frame is gone, while the example cards themselves are unchanged.
* The duplicate mask on the single window-edge gradient has been fixed, preserving the full titlebar fade and reusing its reversed form at the bottom of Lyrics Examples.
* Manual Adjust now uses a draft flow: the song summary slides left as the editor enters from the right. Save applies the draft, Cancel discards it, and both return with the reverse transition.
* The GitHub Actions warning about SHA-pinned Node.js 20 actions being forced onto Node.js 24 remains a separate 5.6.0-cycle evaluation; this change does not modify workflows.
