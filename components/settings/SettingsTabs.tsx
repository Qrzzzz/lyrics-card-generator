...
export function SettingsTabs({ tabs, active, onChange }: { tabs: SettingsTab[]; active: string; onChange: (id: string) => void }) {
  return (
    <nav aria-label="Settings categories" className="flex gap-1 overflow-x-auto border-b border-[rgb(var(--panel-border))] p-2 md:w-48 md:shrink-0 md:flex-col md:border-b-0 md:border-r">
      {tabs.map(({ id, label, icon: Icon }) => (
        <button key={id} type="button" onClick={() => onChange(id)} aria-current={active === id ? "page" : undefined} className="relative flex min-w-max items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm font-semibold">
          {active === id ? <motion.span layoutId="settings-tab" className="absolute inset-0 rounded-lg bg-[rgb(var(--button-bg-hover))]" transition={{ type: "spring", stiffness: 420, damping: 34 }} /> : null}
