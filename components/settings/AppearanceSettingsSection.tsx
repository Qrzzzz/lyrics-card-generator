...
export function AppearanceSettingsSection({ settings, copy, onChange }: { settings: UserSettings; copy: typeof settingsCopy[Locale]; onChange: (settings: UserSettings) => void }) {
  const themeBackground = settings.uiTheme === "light-blue" ? "#EAF6FF"
    : settings.uiTheme === "light-acrylic" ? "#F3F6FA"
    : settings.uiTheme === "dark-acrylic" ? "#141821"
    : "#080910";
  ...
  return <section className="settings-panel-card grid gap-4 p-4 sm:p-5">
    <Label label={copy.theme}><Select value={settings.uiTheme} onChange={(event) => onChange({ ...settings, uiTheme: event.target.value as UserSettings["uiTheme"] })}><option value="album-dynamic">{copy.albumDynamic}</option><option value="light-blue">{copy.lightBlue}</option><option value="dark-pink">{copy.darkPink}</option><option value="dark-acrylic">{copy.darkAcrylic}</option><option value="light-acrylic">{copy.lightAcrylic}</option><option value="custom">{copy.custom}</option></Select></Label>
    {(settings.uiTheme === "dark-acrylic" || settings.uiTheme === "light-acrylic") ? <p className="settings-tip px-3 py-2 text-xs leading-relaxed">{copy.acrylicSupportNote}</p> : null}
